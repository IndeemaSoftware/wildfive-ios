...
- (void)putSymbolOnViewOnPosition:(XBoardPoint) pPoint soundPut:(BOOL)pSound;
...
@synthesize isShowingHint=mIsShowingHint;
...
- (CGPoint)correctedPosition:(CGPoint)pPoint {
    return CGPointMake(pPoint.x - mGameBoardView.frame.origin.x, pPoint.y - mGameBoardView.frame.origin.y - 4.0 * [XGameBoardCell cellSize].height);
}
...
- (void)lockSymbolAtCurrentPosition {
    ...
    [[XSoundEngine sharedEngine] playSoundFixElement];
}
...
- (void)putSymbolOnViewOnPosition:(XBoardPoint) pPoint soundPut:(BOOL)pSound {
    if (mDelegate) {
        if ([mDelegate respondsToSelector:@selector(gameboardCurrentPlayer)]) {
            ...
            [lCell release];
            if (pSound) {
                [[XSoundEngine sharedEngine] playSoundPutElement];
            } else {
                [[XSoundEngine sharedEngine] playSoundChangeElementPosition];
            }
        }
    } else {
...
- (void)highlightSymbolAtPosition:(XBoardPoint) pPoint {
    ...
    for (UIImageView *lView in [mGameBoardView subviews]) {
        if ([lView isKindOfClass:[XGameBoardCell class]]) {
            if ((((XGameBoardCell*)lView).symbolPosition.x == pPoint.x) && (((XGameBoardCell*)lView).symbolPosition.y == pPoint.y)) {
                //puth highlighting code here
                lView.backgroundColor = [UIColor colorWithRed:1.0f green:1.0f blue:1.0f alpha:0.2];
                [lView setHighlighted:YES];
            }
        }
    }
}
...
- (void)putSymbolAtPosition:(XBoardPoint) pPoint {
    [self putSymbolOnViewOnPosition:pPoint soundPut:YES];
    if (mCurrentCell != nil) {
        [mCurrentCell release];
        mCurrentCell = nil;
    }
...
- (void)putSymbolOnViewOnCoordinats:(CGPoint) pPoint {
    ...
    [self putSymbolOnViewOnPosition:XBoardPointMake((int)lPosition.x, (int)lPosition.y) soundPut:NO];
}

- (void)showHintAtPosition:(XBoardPoint) pPoint {
    mIsShowingHint = YES;
    UIView *lCell = [[UIView alloc] initWithFrame:CGRectMake([XGameBoardCell cellSize].width *pPoint.x + 1.0,
                                                             [XGameBoardCell cellSize].height *pPoint.y + 1.0, [XGameBoardCell cellSize].width-1, [XGameBoardCell cellSize].height-1)];
    [lCell setBackgroundColor:[UIColor greenColor]];
    [lCell setAlpha:0.0];
    [mGameBoardView insertSubview:lCell atIndex:0];
    
    [self makeBlink:lCell];
    
    [lCell release];
}

- (void) makeBlink:(UIView*)pView {
    static NSUInteger lState = 0;
    lState ++;
...
- (id) initWithFrame:(CGRect) pFrame {
    ...
    if (self) {
        ...
        if([deviceType() isEqualToString:IPHONE]){
            lRect = CGRectMake(0, 0, 17, 23);
        } else if([deviceType() isEqualToString:IPAD]){
            lRect = CGRectMake(0, 0, 43, 50);
        }
        mHandView = [[UIImageView alloc] initWithFrame:lRect];
        [mHandView setImage:[UIImage imageNamed:@"hand"]];
        [mHandView setHidden:YES];
        [self addSubview:mHandView];
...
- (id) initWithFrame:(CGRect) pFrame boardDimention:(XBoardSize) pBoardSize {
    ...
    if (self) {
        ...
        mGameBoardView = [[XGameBoardView alloc] initWithFrame:CGRectMake((19 - pBoardSize.width) * [XGameBoardCell cellSize].width / 2, 
                                                                          (19 - pBoardSize.height) * [XGameBoardCell cellSize].width / 2, 
                                                                          self.contentSize.width, 
                                                                          self.contentSize.height) 
                                                 withBoardSize:mBoardSize];
        [self addSubview:mGameBoardView];
...
- (void) touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    CGPoint lTouchPoint = [lTouch locationInView:self];
    CGPoint lPosition = [self correctedPosition:lTouchPoint];
    [mHandView setCenter:CGPointMake(lTouchPoint.x + 5, lTouchPoint.y - 40)];
    [mHandView setHidden:NO];
    
    if (![self ignoreTouchAtCoordinates:lPosition]) {
        
        mSymbolPosition = lPosition;
        DLog(@"Location: x:%f  y:%f", lTouchPoint.x, lTouchPoint.y);
